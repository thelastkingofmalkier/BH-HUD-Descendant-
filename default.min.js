...
return null});var tsv="guid\tname\tklassType\telementType\trarityType\tturns\ttypesTargets\tbrag\tminValues\tmaxValues\tmats\tperkBase\tperks\teffects\tpacks";cards.filter(function(c){return!!c}).forEach(function(c){tsv+="\n"+c.guid+"\t"+c.name+"\t"+bh.KlassType[c.klassType].slice(0,2)+"\t"+bh.ElementType[c.elementType][0]+"\t"+bh.RarityType[c.rarityType][0]+"\t"+c.turns+"\t"+c.typesTargets.join("|")+"\t"+String(c.brag)[0]+"\t"+c.minValues.map(function(a){return a.join(",")}).join("|")+"\t"+c.maxValues.join("|")+"\t"+c.mats.join(",")+"\t"+c.perkBase+"\t"+c.perks.join(",")+"\t"+c.effects.join(",")+"\t"+String(c.inPacks)[0]});$("#data-output").val(tsv)})}
...
function mapPlayerHero(hero){var playerHero=player.heroes.find(function(h){return hero.guid==h.guid}),level=playerHero?playerHero.level:"/",hp=playerHero?bh.utils.truncateNumber(playerHero.hitPoints):"/",power=playerHero?playerHero.powerRating+" ":"/";return level+"|"+hp+"|"+power}}
