namespace bh {
	export class PlayerBoosterCard {
		...
		public get rowHtml() { return `<div data-element-type="${this.elementType}" data-type="${this.type}" data-rarity-type="${this.rarityType}">${getImg20("misc", "Boosters")} ${RarityType[this.rarityType][0]}${this.challenge?"*":""} ${this.name} <span class="badge pull-right">${this.count}</span></div>`; }
